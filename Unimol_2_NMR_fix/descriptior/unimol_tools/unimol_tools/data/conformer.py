...
class ConformerGen(object):
    ...
    def single_process(self, molecule):
        if self.method == 'rdkit_random':
            atoms, coordinates = inner_smi2coords(molecule, base_type=self.base_type, seed=self.seed, mode=self.mode, remove_hs=self.remove_hs, no_optimize = self.no_optimize)
    ...
    def single_process_G(self, molecule):
    ...
    def transform(self, smiles_list):
        pool = Pool()
        if isinstance(smiles_list[0],ase.atoms.Atoms):
            logger.info('Get atoms and coords from ASE.Atoms...')
            inputs = [item for item in tqdm(pool.imap(self.single_process_G, smiles_list))]
        else:
...
def inner_smi2coords(molecule, base_type, seed=42, mode='fast', remove_hs=True, no_optimize = True):
    if base_type == 'smiles':
        ...
        raise ValueError('Input must be smiles or mols')
    try:
        atoms = [atom.GetSymbol() for atom in mol.GetAtoms()]
    except:
        raise ValueError('mol has no object GetAtoms')

    # if had conformer
    if mol.GetNumConformers() > 0 and no_optimize:
        coordinates = mol.GetConformer().GetPositions().astype(np.float32)
        return atoms, coordinates
    
    assert len(atoms)>0, 'No atoms in molecule: {}'.format(molecule)
    try:
